...
#include <IOKit/usb/usb_bar_fallback.h>
...
AppleUSBOHCI::UIMInitialize(IOService * provider)
{
	...
	do
	{
		if (!_uimInitialized) 
		{
			...
			if (!_deviceBase)
				_deviceBase = RavynMapPCIMemoryBar(_device, kIOPCIConfigBaseAddress0, 0x1000);
			if (!_deviceBase)
			{
						 ...
						 UInt32			   	 bufferSizeRemaining)
{
    ...
    while (pCurrentTD != pED->pLogicalTailP)
    {
        ...
        pCurrentTD->pShared->currentBufferPtr = 0;
