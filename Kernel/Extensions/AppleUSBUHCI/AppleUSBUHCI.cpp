...
AppleUSBUHCI::UIMInitialize(IOService * provider)
{
    ...
    if (!_uimInitialized) 
	{
		...
		{
            // ravynOS/QEMU: IOPCIFamily doesn't publish the I/O BAR as
            // IODeviceMemory. On x86 UHCI registers are reached through x86 I/O
            // ports (inw/outw on _ioPhysAddress), so recover the I/O BAR base
            // straight from config space; no mapping is required.
            _ioPhysAddress = 0;
            for (uint8_t reg = kIOPCIConfigBaseAddress0; reg <= kIOPCIConfigBaseAddress5; reg += 4)
            {
                uint32_t bar = _device->configRead32(reg);
                if (bar & 0x1) { _ioPhysAddress = (IOPhysicalAddress)(bar & ~0x3u); break; }
            }
            _ioVirtAddress = 0;
            if (!_ioPhysAddress)
            {
                USBError(1, "AppleUSBUHCI[%p]::UIMInitialize - ioMap is NULL and no I/O BAR", this);
                return kIOReturnNoMemory;
            }
            USBLog(1, "AppleUSBUHCI[%p]::UIMInitialize - I/O BAR fallback, port base 0x%x", this, (uint32_t)_ioPhysAddress);
        }
