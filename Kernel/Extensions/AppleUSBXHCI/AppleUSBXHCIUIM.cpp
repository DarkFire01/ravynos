...
UInt16 AppleUSBXHCI::Read16Reg(volatile UInt16 *addr)
{
    ...
    if (!_lostRegisterAccess)
    {
		// ravynOS/QEMU: the emulated XHCI register MemoryRegions only honor
		// 32-bit accesses; a 16-bit MMIO read returns 0. Read the containing
		// aligned 32-bit word and extract the appropriate half. Harmless on
		// real hardware (these 16-bit fields share a 32-bit word with no
		// read side effects).
		uintptr_t          a    = (uintptr_t) addr;
		volatile UInt32 *  word = (volatile UInt32 *)(a & ~((uintptr_t)3));
		UInt32             w    = OSReadLittleInt32(word, 0);
		value = (a & 2) ? (UInt16)(w >> 16) : (UInt16)(w & 0xFFFF);
...
#pragma unused(source)
	...
	{ static int _rdbg = 0; if (_rdbg++ < 3) kprintf("RAVYNDBG XHCI PrimaryInterruptFilter fired #%d\n", (int)controller->_numPrimaryInterrupts); }
...
IOReturn AppleUSBXHCI::UIMInitialize(IOService * provider)
{
    ...
    do {
        ...
        {
            // ravynOS/QEMU: IOPCIFamily doesn't always publish IODeviceMemory for
            // a device's BARs, so map BAR0 (the XHCI MMIO register space) from the
            // physical address read straight out of config space, as the ravynnvme
            // and RavynAHCIPort drivers do.
            uint32_t bar0 = _device->configRead32(kIOPCIConfigBaseAddress0);
            uint64_t phys = 0;
            if (!(bar0 & 0x1))                     // memory BAR
            {
                if ((bar0 & 0x6) == 0x4)           // 64-bit BAR
                {
                    uint32_t bar1 = _device->configRead32(kIOPCIConfigBaseAddress0 + 4);
            ...
            if (phys)
            {
                IOMemoryDescriptor * md = IOMemoryDescriptor::withPhysicalAddress(
                        (IOPhysicalAddress) phys, 0x10000,
                        kIODirectionInOut | kIOMemoryMapperNone);
                if (md)
                {
                    _deviceBase = md->map(kIOMapAnywhere | kIOMapInhibitCache);
                    md->release();
                }
            }
            if (_deviceBase)
            {
                USBLog(1, "AppleUSBXHCI[%p]::UIMInitialize - mapped BAR0 via physical fallback (0x%llx)", this, phys);
            }
        }
        if (!_deviceBase)
        {
		...
		if( (_errataBits & (kXHCIErrataPPT | kXHCIErrata_FrescoLogic | kXHCIErrata_ASMedia |kXHCIErrata_Etron7052)) == 0)
		{
			...
			{
				kprintf("RAVYNDBG XHCI UIM - controller REJECTED (AllowAnyXHCI not set); errata=0x%x\n", (unsigned int)_errataBits);
		...
		{
			uint16_t hciv2  = Read16Reg(&_pXHCICapRegisters->HCIVersion);
			uint16_t raw16  = *(volatile uint16_t *)((volatile uint8_t *)_pXHCICapRegisters + 2);
			uint32_t raw32  = *(volatile uint32_t *)_pXHCICapRegisters;
			kprintf("RAVYNDBG XHCI UIM - raw32=0x%x HCIVer1=0x%x HCIVer2=0x%x raw16@2=0x%x lost=%d\n",
					raw32, (unsigned int)HCIVersion, (unsigned int)hciv2, (unsigned int)raw16, (int)_lostRegisterAccess);
		}
		if(_lostRegisterAccess)
		{
			err = kIOReturnNoDevice;
		...
		kprintf("RAVYNDBG XHCI UIMInitialize - SUCCESS\n");
		return kIOReturnSuccess;
	...
	kprintf("RAVYNDBG XHCI UIMInitialize - FAILED err=0x%x\n", err);
	UIMFinalize();
										 ...
										 short				direction)
{
    ...
    SetDCBAAAddr64(&_DCBAA[slotID], 0);
