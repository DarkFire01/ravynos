	...
	if (_controllerAvailable && !_wakingFromHibernation && !_lostRegisterAccess)
	{
		...
		if (GetRootHubStatus((IOUSBHubStatus *)&tempStatus) == kIOReturnSuccess)
		{
			...
			for (port = 1; port <= _rootHubNumPorts; port++)
			{
				...
				{ static int _rdbg = 0; if (_rdbg < 80) { _rdbg++; kprintf("RAVYNDBG XHCI RHStatus port %d ss=%d ret=0x%x status=0x%04x change=0x%04x\n", (int)port, (int)portIsSuperSpeed, (unsigned)retStatus, (unsigned)portStatus.statusFlags, (unsigned)portStatus.changeFlags); } }
