...
IOUSBControllerV3::start( IOService * provider )
{
	...
	if (_device == NULL)
	{
		kprintf("RAVYNDBG XHCIv3::start - provider not IOPCIDevice\n");
		goto ErrorExit;
	}
	kprintf("RAVYNDBG XHCIv3::start - entered, speed=%d\n", (int)_controllerSpeed);
	...
	kprintf("RAVYNDBG XHCIv3::start - calling super::start\n");
	if ( !super::start(provider))
	{
		kprintf("RAVYNDBG XHCIv3::start - super::start FAILED\n");
		goto ErrorExit;
	}
	kprintf("RAVYNDBG XHCIv3::start - super::start OK\n");
	...
	// initialize PM
	kprintf("RAVYNDBG XHCIv3::start - rootHubTimer OK, calling InitForPM\n");
	err = InitForPM();
	if (err != kIOReturnSuccess)
	{
		kprintf("RAVYNDBG XHCIv3::start - InitForPM FAILED err=0x%x\n", err);
	...
	kprintf("RAVYNDBG XHCIv3::start - SUCCESS, returning true\n");
	return true;
...
IOUSBControllerV3::RHCompleteTransaction(IOUSBRootHubInterruptTransactionPtr outstandingRHTransPtr)
{
	...
	{ static int _r=0; if (_r<20){_r++; kprintf("RAVYNDBG RHCompleteTransaction listener=%d bitmap=0x%x\n", outstandingRHTransPtr[0].completion.action?1:0, (unsigned)_rootHubStatusChangedBitmap); } }
...
IOUSBControllerV3::CheckForRootHubChanges(void)
{
	...
	{ static int _r=0; if (_r<20){_r++; kprintf("RAVYNDBG CheckForRootHubChanges bitmap=0x%x speed=%d\n", (unsigned)_rootHubStatusChangedBitmap, (int)_controllerSpeed); } }
